...
app.get('/player_info',function(req,res) {
	var list_players = 'select * from football_players;';
	db.any(list_players)
        .then(function (rows) {
...
app.get('/player_info/post', function(req,res){
	...
	var p_options =  'select * from football_players;';
	var p_img = "select img_src from football_players WHERE id ='" + p_choice + "';";
	var p_selected = "select * from football_players WHERE id ='" + p_choice + "';";
	var p_gameplayed = "SELECT COUNT(*) from football_games WHERE players @> ARRAY["+p_choice+"];"
	db.task('get-everything', task => {
        return task.batch([
            task.any(p_options),
						task.any(p_img),
						task.any(p_selected),
						task.any(p_gameplayed),
        ]);
    })
    .then(info => {
			res.render('pages/player_info',{
